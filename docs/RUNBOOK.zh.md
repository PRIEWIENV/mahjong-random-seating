...
**管理台是一张卡片一张卡片各自刷新的**，每张按自己的节奏——会动的五秒一次，冻结之后不可能变的
指纹干脆不刷。装着这两个表单的那张卡片，正是在表单可用期间停住不刷的那一张，因为刷新会替换掉卡片
的内容，把填到一半的替补声明清空；轮次一锁定它就恢复，因为那时候才真的有东西要看。反正这个状态下
什么也不会自己变。

**你可以自己排版。** 每张卡片有 ↑ ↓ 调位置、↔ 切换整宽/半宽、× 移除；移除的卡片从顶部那条带子里
点回来，卡片的标题栏本身就是拖拽把手。排版只记在这个浏览器里，别处都没有——它是一个人在一块屏幕
前的偏好，不是赛事带着走的设置。

**有一张日志卡片。** 显示这台服务器自己的输出，最近 500 行，开奖任务和决赛抽签任务打印的东西都
在里面。要分辨「信标迟到了」和「任务死了」，靠的就是它，不用再 ssh 上去。日志只在内存里，重启就
没了。
...
19. [ ] 什么都不用做。信标落地时服务器会自己抽签，用的是跟第一次开奖同一个定时器。看 `/admin`——表单没了之后，决赛轮那张卡片每五秒刷新一次——状态会从**已锁定**变成**已抽签**，十二个座位也会直接显示在那张卡片上，那就是要念给场地的那张表。选手们在结果页上看的是同一件事：那里已经出现了一条到信标的倒计时，座位表的第十二列也已经留了出来
