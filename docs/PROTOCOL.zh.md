...
**T2 不需要任何人去跑。** 它不贡献任何属于它自己的东西——桌次来自一份在信标存在之前就已公布的锁定文件，风位来自一份在比赛开始之前就已冻结的脚本，随机性来自锁定文件点名的那一轮——所以其中没有任何决定，也就没有谁需要到场。服务器用跟第一次开奖同一个定时器跑它（`server/schedule.js`），门禁是三个文件检查加一个时钟：锁定文件存在、`final.json` 不存在、信标已到期。那是一个定时器而不是一个端点，所以 §9 在这里和在第一次开奖那里一样不受影响。指望组织者在比赛的两轮之间去开一个终端，正是决赛轮不会发生的原因。
