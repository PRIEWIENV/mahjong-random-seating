...
6. **每一对关系都是平衡的。** 对任意一对选手，他们的三次相遇应当由一次对坐、一次前者在上家、一次后者在上家构成。如下图所示，尽量排除因位置而造成的又是或者劣势。
...
组合学里的理想性质清单清单常有崩塌的习惯，这六条里有两条被证明无法实现。
...
两点实务说明。十二人中至少八人提交，抽签即开——因为一份诚实的贡献已经足以让结果不可预测，主要是考虑到实际操作中的容错，不能将整个系统的可用性押在所有人都准时抽签上。

## 从十二份提交到一张座位表

这一节把整个计算过程一步一步写完，并给出确切的公式。这里的每一步都是确定性的：同样的输入必须产生逐字节相同的座位表，否则两个人各自核验就会得到不同的答案，前面说的「可验证」也就一文不值了。
...
这个 nonce 就是「选 7 的人和掷骰子的人贡献一样多不可预测性」的原因：浏览器那 16 个随机字节也在里面。它同时也让任何人无法从密文猜出提交内容——没有它，别人可以把 256 个可能的数字全都加密一遍来比对。

这份载荷随后被时间锁加密到目标信标轮次，密文立即公开。立即公开正是把选手**绑定**在这份提交上的那一步。
...
其中 `signature` 是 drand 在目标轮次的 [BLS 签名](https://en.wikipedia.org/wiki/BLS_digital_signature)，也正是打开那些密文的那个值。把它折进来，意味着抽签并不只依赖那十二个浏览器：即使全部选手串通，他们在提交的时候也仍然不知道这个信标值。而把 `local_ids` 包含进来，意味着参与者集合不同就是另一次抽签，哪怕大家填的数字完全一样。

### 第 5 步——把种子变成随机数

种子是 32 个字节，而洗牌需要的是一条流。这条流是计数器模式下的 SHA-256：
...
读作：模板 0 号点是 8 号选手，1 号点是 10 号选手，2 号点是 9 号选手，依此类推。第 7 步再去读模板——第 1 轮第 1 桌是 0、1、2、3 号点，按东南西北顺序——于是第 1 轮里 8 号选手坐第 1 桌东家，10 号选手南家，9 号选手西家，6 号选手北家。
