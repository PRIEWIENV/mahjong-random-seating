...
const TEXT = {
  zh: {
    ...
    ledeDone: (n) => `下面是本次开奖所依据的 ${n} 份密文，连同它的指纹与存证：`,
    ask: '把它发到群里，和别人核对一遍。',
    why: '现在还没有人能解开任何一份密文——解密要等的那一轮信标尚未产生。所以此刻公布这份结果，等于承诺了参与者就是这些人，而且承诺时谁也不知道换个人会带来什么结果。十二个人看到的指纹必须一样。',
    whyDone: '上方是本次所有提交并被打开的密文的指纹，下方是这批密文的存证，任何人现在都可以下载下来自己解开、自己复算。',
    copy: '复制',
    copied: '已复制',
    proofTitle: '存证与验证',
    anchored: (n) => `这份文件已由 ${n} 个独立的日历服务器写进比特币区块链做了时间戳存证，证明它在某个区块之前就已存在，而不是事后伪造。`,
    notAnchored: '外部时间戳存证这次没能完成。指纹本身仍然有效，所以和别人核对这一步这次格外重要。',
    fileRoll: '下载密文提交结果',
    fileProof: '下载时间戳证明',
    howTitle: '自己验证（不需要命令行）：',
    how: [
      '把上面两个文件都下载下来。',
  ...
  en: {
    ...
    ledeDone: (n) => `Below are the ${n} ciphertexts this draw was computed from, with their fingerprint and proof:`,
    ...
    whyDone: 'Above is the fingerprint of every ciphertext submitted and opened for this draw; below is the proof for that same set. Anyone can now download them, open them, and recompute the result.',
    copy: 'Copy',
    copied: 'Copied',
    proofTitle: 'Proof, and how to check it',
    anchored: (n) => `${n} independent calendar servers timestamped this file into the Bitcoin blockchain, proving it existed before a particular block rather than being written afterwards.`,
    notAnchored: 'The external timestamp did not go through this time. The fingerprint still holds, so comparing it with other people matters more than usual.',
