...
实际上组织者是在 **`/admin`** 里做这一步的，因为第十二轮紧接着第十一轮开始，
场馆里没人坐在终端前。面板把这几条命令作为子进程跑一遍并把它们说的话显示出来，
它不是第二份实现。服务器没在跑的时候，这几条命令仍然是傅底。
...
`--in 5m` 是默认值。它曾经是四十五分钟，选这个数是为了「显然够用」；实测之后发现它是
技术需求的三百多倍：整个发布序列——写入、镜像、drain、盖戳、镜像 .ots、再 drain——不到十秒，
其中向四个 calendar 做一次 OpenTimestamps 往返约两秒。而那四十五分钟不是白给的：
第十二轮紧接着第十一轮，所以那段间隔就是最后一份成绩录完到座位贴出之间的死等，
而大家已经坐下了。

这段间隔买到的是人的互相核对：十二个人在谁都还不知道它会开出什么的时候核对同一串摘要。
五分钟给这件事留了一个真实可用的窗口。但有一点值得说清楚：互相核对是双保险——
证明锁定早于信标的**证据**是机器产生的，根本不需要任何人的注意力：那是各 calendar 的签认和
镜像的 commit 时间戳，两者组织者都无法回拨。

这个数字也不再是承重的了，因为工具现在是**实测**余量而不是预先假定：它会报告实际剩下多少时间，
不足三十秒会警告，而如果发布超过了它自己的信标则直接拒绝。一个宽裕的提前量从来只是
「发布很快」这个假设，而一个卡住六分钟的 calendar 会静默地打破它。底线仍然是一分钟。

**然后把它打印的摘要通告出去**，连同 drand 轮次和预计出块时间。这就是全部证据：名次在给它们
排座位的那份随机性存在之前就已固定，而十二个人手里握着同一串短字符串，正是这件事可被核验的
原因。那一轮落地之后，同一串摘要就什么也证明不了了。

**如果有人中途退赛**，在事情发生的当时就声明，而不是等到锁定的时候。替补沿用该座位原有的
Pantheon 注册位——不要把他登记成第十三个人，因为名次表是按**打过的场次**生成的，
单独注册的替补会成为同一个座位的第二行残缺记录。在 `/admin` → 声明替补 里填，它写的是
`data/substitutes.json`，而这个文件：

- **不冻结，也不可能冻结**——退赛发生在打 tag 之后好几周；
- 和 `protocol.json`、`roster.json` 一样被 **gitignore**，因为它是某一场赛事的数据。
  它通过被抄进 `events/final/lock.json` 而公开，而后者会被镜像并盖上时间戳，
  所以公开的副本只有一份，不存在两份不一致的可能；
- 会被 `tools/end-event.js` **归档并清除**。这一条很要紧：下一次冻结会重写 `roster.json`
  和 `protocol.json`，却不会动这个文件，于是它会跟着 checkout 进入下一届比赛，
  把上一届的替补算到一个他从没打过的座位上。`tools/freeze.js` 会拒绝为一个已有声明的
  赛事打 tag，因为冻结的时刻还没有人打过一场球。

这些都不会影响抽签——座位保留了注册位，所以名次表、名次段和种子都与没换人时完全一致。
为什么正是这一点使得一次晚到的声明是安全的而不只是方便的，见 PROTOCOL.zh.md §11.6。

**抽签不需要任何人。** 信标落地时服务器会自己跑它，用的是跟第一次开奖同一个定时器，
门禁是三个文件检查加一个时钟：锁定文件存在、`final.json` 不存在、信标已到期。那是一个定时器
而不是一个端点，所以 §9「任何外部人能戳到的东西都不得触发开奖」这条不受影响。看 `/admin`：
状态从**已锁定**变成**已抽签**，座位也会出现在结果页上。

其中没有任何需要做的决定——桌次来自一份在信标存在之前就已公布的锁定文件，风位来自一份在
比赛开始之前就已冻结的脚本——所以没有谁需要到场，而比赛的第十二轮紧接着第十一轮开始，
那个时候本来就没人坐在终端前。

手动跑（服务器没在跑的时候，或者想当着大家的面跑）：
