...
**Read [`docs/IMPLEMENTATION_NOTES.md`](docs/IMPLEMENTATION_NOTES.md) before freezing.** Two items there change what goes into `protocol.json` and what "verified" means:

1. `protocol.json` needs a `chain_public_key` field, which §4 does not list. Without it `drand-client` verifies nothing at all rather than failing loudly.
2. `results.json` cannot be *fully* reproducible, because §4 puts the Pantheon sync outcome inside a file that RUNBOOK A5 also requires to reproduce byte for byte. `--verify` checks the reproducible core and names what it excluded.
...
```sh
npm ci
npm test                  # 72 unit tests, offline, ~3s
...
data/
  ...
  protocol.example.json      # drand chain, target round, quorum; fill in, rename to protocol.json
server/
...
3. **The quorum rule is frozen too** (see `PROTOCOL.md` §8). It must not be renegotiated when a 7-of-12 situation actually arises — deciding after the fact is itself a manipulable step.
4. **What a player submitted is never exposed before the reveal.** Only whether they submitted.
5. **Sync to Pantheon with `WIND_SHUFFLE_MODE_PRESCRIPTED`.** Any other mode re-randomises the winds and throws away most of what the template was optimised for.
